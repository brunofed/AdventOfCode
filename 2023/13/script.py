...
def find_horizontal_symmetry(pattern, quasi_symmetry=False):
    n = len(pattern)
    for i in range(1, n):
        double = 2 * i
...
def find_symmetry(pattern, quasi_symmetry=False):
    row = find_horizontal_symmetry(pattern, quasi_symmetry)
    pattern_transposed = np.array(pattern).T.tolist()
    column = find_horizontal_symmetry(pattern_transposed, quasi_symmetry)
    return row, column


def problem(input, quasi_symmetry=False):
    sum = 0
    # store_results = []
    for pattern in input:
        row, column = find_symmetry(pattern, quasi_symmetry)
        if row + column == 0:
            raise ValueError(f"no {"quasi" if quasi_symmetry else ""}symmetry found in pattern:\n{pattern}")
        sum += 100 * row + column
...
#                         if pattern[i][j] != modified_pattern[i][j]
#                     ]
#                 )
#                 == 1
...
if __name__ == "__main__":
    for filename in ["input_example", "input"]:
        ...
        # result1, store_results = problem(input)
        result1 = problem(input)
